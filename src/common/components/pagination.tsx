...
const Pagination = ({ allItems, currentItems = 0, limit, offset, newPage, prevPage, setOffset, params = true }: IPagination) => {
  ...
  useEffect(() => {
    if (params && searchParams.get('page')) {
      setOffset((Number(searchParams.get('page')) - 1) * limit);
    }
  }, [limit, params, searchParams, setOffset])
  ...
  return (
    <nav className="flex w-full gap-2 items-center flex-column flex-wrap md:flex-row justify-between" aria-label="Table navigation">
      ...
      >
        Mostrando <span className="font-semibold text-light-text-primary dark:text-dark-text-primary">{offset + 1}-{offset + currentItems}</span> de <span className="font-semibold text-light-text-primary dark:text-dark-text-primary">{allItems}</span>
      </span>
